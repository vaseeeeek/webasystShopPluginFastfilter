var reviews = {
    ...
    send: function () {
        ...
        $(".js-filters.js-ajax form").submit(function () {
            var form = $(this);
            return itemList.ajaxUpListForm(form.serializeArray()), !1;
        });
    },
            ...
            if (!$(this).find(".js-slider-range").length) {
                ...
                if (r.data("step")) t = parseFloat(r.data("step")); else {
                    ...
                    if (Math.round(e) != e || Math.round(i) != i) {
                        ...
                        t = Math.pow(10, -o), (o = Math.round(1e5 * Math.abs(Math.round(e) - e)) / 1e5) && o < т && (т = о), (о = Math.round(1e5 * Math.abs(Math.round(i) - i)) / 1e5) && o < т && (т = о);
                    }
                }
                r.slider({
                    range: !0,
...
$(document).ready(function () {
    ...
    $(function () {
        ...
        window.seofilterOnFilterSuccessCallbacks.push(function () {
            $(".js-filter-body").find('input[type="checkbox"], input[type="radio"], .js-select').trigger("refresh");
            categoryText.moreDetails();
            bindFilterChangeEvents();  // Привязка событий к новым элементам фильтра
        });
    });
}), $(function () {
